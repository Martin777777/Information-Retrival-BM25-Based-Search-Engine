...
def indexing():
    ...
    # get filenames of all the files under the documents dicrionary
    dictionary_names = os.listdir("documents")

    files_number = 0
    total_length = 0
    document_lengths = {}

    # traverse all the files under the documents dictionary
    for dictionary_name in dictionary_names:
        filenames = os.listdir("documents/" + dictionary_name)
        for filename in filenames:
            with open("documents/" + dictionary_name + "/" + filename, "r", encoding="utf-8") as document:
...
# This function aims to read the file qrels.txt and get all judged documents with their similarity scores for queries
# and all documents which are judged relevant for queries with their similarity scores.
def load_qrels():
    # all_documents_dict is to store all judged documents with their similarity scores for queries.
    all_documents_dict = {}
    # relevant_documents_dict is to store all documents
    # which are judged relevant for queries with their similarity scores.
...
def bpref(query_id, output):
    # All judged documents with their similarity scores for queries are gotten here.
    global all_documents
    documents = all_documents[query_id]
    # judged stores the documents that are judged in the file qrels.txt.
    # The documents that do not appear in the file qrels.txt are unjudged documents and the documents whose similarity
    # scores are 0 are irrelevant document.
    judged = documents.keys()
...
def NDCG_at_10(query_id, output):
    # All documents which are judged relevant for queries with their similarity scores are gotten here.
    global relevant_documents_set
...
def evaluation():
    ...
    with open("files/queries.txt", "r") as f:
        ...
        for line in f:
            ...
            # results (the number is 15) but other scores for other metrics increase significantly.
